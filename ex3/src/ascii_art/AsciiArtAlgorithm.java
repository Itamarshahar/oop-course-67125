...
/**
 * The AsciiArtAlgorithm class converts an image to ASCII art using a
 * specified character set and resolution.
 * It divides the image into smaller sub-images, calculates their brightness,
 * and matches each sub-image to a character
...
public class AsciiArtAlgorithm {
    ...
    /**
     * Constructs an AsciiArtAlgorithm with the given image, character set,
     * and resolution.
     *
    ...
    public char[][] run() {
        ...
        for (int col = 0; col < resolution; col++) {
            for (int row = 0; row < resolution; row++) {
                ...
                double brightness =
                        image.calculateImageBrightness(subImages.get(index));
                asciiArt[row][col] =
